...
def _check_ownership(session: RoleplaySession, user: User):
    """Allow access if user owns the session or is admin."""
    if user.role == UserRole.admin.value:
        return
    if session.user_id and str(session.user_id) != str(user.id):
...
def start_session(
    ...
    log_activity(db, current_user.id, "roleplay.start", {"product": product.name})
...
def send_message(
    ...
    session = db.query(RoleplaySession).filter(RoleplaySession.id == data.session_id).first()
    if not session:
...
def end_session(
    ...
    log_activity(db, current_user.id, "roleplay.complete", {"score": evaluation["overall_score"]})
...
def get_history(
    ...
    """Get past roleplay sessions — filtered by user, admin sees all."""
    query = db.query(RoleplaySession).order_by(RoleplaySession.created_at.desc())
    if current_user.role != UserRole.admin.value:
        query = query.filter(RoleplaySession.user_id == current_user.id)
    sessions = query.limit(20).all()
...
def sahayak_help(
    data: SahayakHelpRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Get quick product help from Sahayak during roleplay."""
    from app.services import agent_service

    result = agent_service.chat(message=data.question, db=db, session_id=data.session_id)
    return {
        "response": result["response"],
...
def delete_all_sessions(
    ...
    """Delete all roleplay sessions for the current user. Admin deletes all."""
    query = db.query(RoleplaySession)
    if current_user.role != UserRole.admin.value:
        query = query.filter(RoleplaySession.user_id == current_user.id)
    count = query.delete()
    db.commit()
